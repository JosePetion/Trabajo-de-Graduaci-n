function eu_dist = euclidian(population, pop_size, n_points)
    ...
    for i = 1:pop_size
        ...
        for j = 1:(size(distances,1))
            distances(j) = sqrt((points_array(j+1, 1) - points_array(j, 1))^2 + (points_array(j+1, 2) - points_array(j, 2))^2) + (points_array(j+1, 3) - points_array(j, 3))^2;
        end 
        e_distance(i, :) = distances';
    end
    eu_dist = sum(e_distance, 2);
end